<div class="container mx-auto">
    ...
    <table class="min-w-full border-collapse border border-gray-400 mt-4">
        ...
        <tbody>
            ...
            <tr>
                <td class="py-2 px-4 border border-gray-300">{{ $index + 1
}}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $pegawai->nip
}}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $pegawai->nama
}}</td>
                <td class="py-2 px-4 border border-gray-300">{{
$pegawai->nama_unit_kerja }}</td>
                <td class="py-2 px-4 border border-gray-300">
                    <flux:button :href="route('pegawai.edit', $pegawai)">Edit</flux:button>
                    <flux:button variant="danger" wire:click="delete({{
$pegawai->id }})" wire:confirm="Are you sure?">Delete</flux:button>
                </td>
            </tr>
            @endforeach
        </tbody>
